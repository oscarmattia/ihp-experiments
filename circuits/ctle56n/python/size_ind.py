...
class IndShuntModel:
    ...
    gds: str
    solver: str
    cox_from_rule_of_thumb: bool = False
    r_from_dc_fallback: bool = False
...
def _build_ind_shunt_model_from_lut(
    ...
    cox_p, cox_n, c_plate, c_rule, cox_fallback = pimodel_parasitics(
    ...
    return IndShuntModel(
        ...
        cox_from_rule_of_thumb=cox_fallback,
        r_from_dc_fallback=r_fallback,
    )


def _header_lines(m: IndShuntModel, subckt_name: str = "ind_shunt") -> list[str]:
    skin_ratio = m.r_28g_ohm / m.r_dc_ohm if m.r_dc_ohm > 0 else float("nan")
    lines = [
        f"* {subckt_name} — generated by size_ind.py (EM lumped TopMetal2 1-turn octagon)",
    ...
    if m.r_from_dc_fallback:
        lines.append(
            f"* Skin ladder: R1={m.r_skin_r1_ohm:.4f} Ohm || (R2={m.r_skin_r2_ohm:.4f} Ohm"
            f" + Ls={m.l_skin_h*1e12:.3f} pH); R@28GHz = R_dc = {m.r_28g_ohm:.4f} Ohm"
        )
    else:
        lines.append(
            f"* Skin ladder: R1={m.r_skin_r1_ohm:.4f} Ohm || (R2={m.r_skin_r2_ohm:.4f} Ohm"
    ...
    if m.cox_from_rule_of_thumb:
        lines.append(
            f"*   Cox fallback: LUT has no C_PORT/pimodel; used rule-of-thumb"
            f" ~{m.c_rule_thumb_f*1e15:.1f} fF (7.5 fF per 100 pH)"
        )
    if m.r_from_dc_fallback:
        lines.append(
            "*   R fallback: LUT has no R_SERIES; R@28GHz = R_dc from sheet R"
            " (no EM skin rise); skin ladder fit from (R_dc, R_dc)"
        )
    return lines


def write_ind_shunt_inc(
    model: IndShuntModel,
    path: Path,
    subckt_name: str = "ind_shunt",
) -> Path:
    path = Path(path)
    lines = _header_lines(model, subckt_name=subckt_name)
    lines += [
        f".subckt {subckt_name} p n sub",
...
def generate_ind_shunt(
    ...
    write_ind_shunt_inc(model, out, subckt_name=subckt_name)
    return model
