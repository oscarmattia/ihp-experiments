...
DEFAULT_OUT = Path(__file__).resolve().parents[1] / "spice" / "vga_params.inc"


def write_vga_params_inc(params: VgaParams, path: Path) -> None:
    """Emit spice/vga_params.inc from the same tokens as extra_params()."""
    d = extra_params(params)
    lines = ["* VGA parameters — generated by size_vga.py"]
    for k, v in d.items():
...
def main() -> None:
    ...
    write_vga_params_inc(params, DEFAULT_OUT)
