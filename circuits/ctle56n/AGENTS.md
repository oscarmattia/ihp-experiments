...
**Layout is in progress** for `term_dut`, `vga_dut` and `driver_dut` (see
`layout/blocks/AGENTS.md` for gate status). Floorplans are still being iterated.

That restructure is also what makes post-layout simulation cheap. An extracted CTLE presents the same
seven pins, so `python/stage_postlayout.py --dut <path> --pass-name <name>` feeds it through the same
`tb_*.cir` templates and writes `out/<name>/` alongside `out/ideal` and `out/pdk`, comparable metric
for metric. It takes the netlist **by path**, so nothing here imports from `layout/`; the netlists are
generated by `layout/blocks/run_postlayout.py`. `run.sh --with-postlayout <path>` is the opt-in hook.
