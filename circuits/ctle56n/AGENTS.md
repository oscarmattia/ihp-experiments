...
| Analyses | DC OP, AC to 300 GHz (diff/CM/PSRR), Zin/S11, PRBS9 transient + eye, SBR taps |
...
Per-pass directories `out/{term,ideal,pdk,vga_ideal,vga_pdk,chain}/` each carry `metrics.csv`, `op.txt`,
`ac_diff.png`/`.csv`, `cmrr.png`, `psrr.png`, transient and eye PNG/CSV, and `sbr.png`/`sbr.csv`/
`sbr_taps.csv`. Termination and chain additionally carry `zin.png`/`.csv`. `out/summary.csv` aggregates
every pass; `ctle_report.md` is auto-regenerated. `out/**/work/` is gitignored ngspice scratch.
