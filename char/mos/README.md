...
- Upstream `pygmid` ngspice netlists hardcode LV probe names, so `ihp_sweep.py` generates IHP-aware netlists and writes **pygmid-compatible** `.pkl` LUTs **plus** portable `.npz` via `char.common.lut`.
- `pygmid` itself is installed in the EDA venv for later `Lookup` usage on the pickle tables.
...
# or everything (MOS + BJT):
./char/run_all.sh
```

## Outputs (`char/mos/out/`)

- `{family}_n.pkl` / `{family}_p.pkl` — pygmid-compatible LUTs
...
- **pygmid Lookup:** load a pickle with `from pygmid import Lookup` after installing pygmid into the EDA venv.
