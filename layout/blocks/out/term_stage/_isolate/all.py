...
from layout.common.rules import route_width, min_space
...
def build_term_stage(params: dict[str, float] | None = None) -> Block:
    ...
    for side, pad_net, pad_cx, pad_dx, trunk_x in (
        ("p", "inp", pad_cx_p, snap(pad_p_box.left), trunk_inp_x),
        ("n", "inn", pad_cx_n, snap(pad_n_box.left), trunk_inn_x),
    ):
        ...
        # ESD PAD/VDD/VSS share Metal2 at the device row; a horizontal run toward the
        # bond pad crosses the rail terminals (driver_stage drops to a feed y first).
        rect(layout, cell, PAD_FEED_METAL, pad_cx - pad_feed_w / 2, pad_top,
              pad_cx + pad_feed_w / 2, bus_top)
        rect(layout, cell, PAD_FEED_METAL, min(pad_cx, trunk_x) - pad_feed_w / 2, bus_top - pad_feed_w / 2,
              max(pad_cx, trunk_x) + pad_feed_w / 2, bus_top + pad_feed_w / 2)
        via_between(layout, cell, trunk_x, bus_top, PAD_FEED_METAL, PORT_METAL)
        for esd_t in (evdd_t, evss_t):
            px, py = esd_t["PAD"].center
            via_between(layout, cell, px, py, "Metal2", PAD_FEED_METAL)
            _edge_route(layout, cell, PAD_FEED_METAL, px, py, bus_top, pad_feed_w)
            if abs(px - pad_cx) > 1e-6:
                rect(layout, cell, PAD_FEED_METAL, min(px, pad_cx) - pad_feed_w / 2, bus_top - pad_feed_w / 2,
                      max(px, pad_cx) + pad_feed_w / 2, bus_top + pad_feed_w / 2)
    ...
    if _bisect_at_least("plus_clamp") and clamp_t:
        ...
        clamp_vss_y = clamp_t["VSS"].center[1]
        via_between(layout, cell, tap_x, tap_y, "Metal1", "Metal3", columns=1, rows=1)
        rect(layout, cell, "Metal3", min(tap_x, vss_bus_x) - m3_w / 2, tap_y - m3_w / 2,
              max(tap_x, vss_bus_x) + m3_w / 2, tap_y + m3_w / 2)
        rect(layout, cell, "Metal3", vss_bus_x - m3_w / 2, min(tap_y, clamp_vss_y),
              vss_bus_x + m3_w / 2, max(tap_y, clamp_vss_y))
    else:
        vss_bus_x = snap(side_x - 2.0)
        vdd_bus_x = snap(side_x - 2.0 - max(m3_sep, 3.0))
    ...
    if _bisect_at_least("plus_divider"):
        ...
        for terminal, bus_x, strap_y, bottom, bus_w in (
            ...
            (cdec_t["MINUS"], vss_bus_x, vss_strap_y, "Metal3", m3_w),
        ):
    ...
    if _bisect_at_least("plus_ring"):
        ...
        if _bisect_at_least("plus_divider"):
            rect(layout, cell, "Metal2", min(vdd_bus_x, axis) - m2_w / 2, vdd_strap_y - m2_w / 2,
                  max(vdd_bus_x, axis) + m2_w / 2, vdd_strap_y + m2_w / 2)
        ...
        if clamp_t:
            lx, ly = via_up(layout, cell, clamp_t["VDD"], "Metal2")
            rect(layout, cell, "Metal2", min(lx, vdd_bus_x) - m2_w / 2, ly - m2_w / 2,
                  max(lx, vdd_bus_x) + m2_w / 2, ly + m2_w / 2)
            _edge_route(layout, cell, "Metal2", vdd_bus_x, ly, vdd_strap_y, m2_w)
            lx, ly = via_up(layout, cell, clamp_t["VSS"], "Metal3")
            rect(layout, cell, "Metal3", min(lx, vss_bus_x) - m3_w / 2, ly - m3_w / 2,
                  max(lx, vss_bus_x) + m3_w / 2, ly + m3_w / 2)
            _edge_route(layout, cell, "Metal3", vss_bus_x, ly, vss_strap_y, m3_w)
