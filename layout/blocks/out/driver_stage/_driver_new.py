...
def build_driver_stage(params: dict[str, float] | None = None,
    ...
    poly = lm["gatpoly_drw"]
    poly_overhang = snap(min(0.17, rule("Gat_d") + 0.08))
    cell.shapes(layout.layer(poly[0], poly[1])).insert(
        pya.DBox(nmos_left, snap(gate_y - poly_overhang), nmos_right, snap(gate_y + poly_overhang))
    )
    ...
    clamp_tap_x = snap(axis)
    clamp_vss_x = snap(nmos_right + VIA_OFFSET)

    if clamp_t is not None:
        vdd_tx, vdd_ty = via_up(layout, cell, clamp_t["VDD"], "Metal2")
        rect(layout, cell, "Metal2", min(vdd_tx, clamp_tap_x) - m3_w / 2, vdd_ty - m3_w / 2,
              max(vdd_tx, clamp_tap_x) + m3_w / 2, vdd_ty + m3_w / 2)
        _edge_route(layout, cell, "Metal2", clamp_tap_x, vdd_ty, vdd_y, m3_w)
        via_between(layout, cell, clamp_tap_x, vdd_y, "Metal2", "TopMetal2", columns=1, rows=1)

        vss_tx, vss_ty = via_up(layout, cell, clamp_t["VSS"], "Metal2")
        rect(layout, cell, "Metal2", min(vss_tx, clamp_vss_x) - m3_w / 2, vss_ty - m3_w / 2,
              max(vss_tx, clamp_vss_x) + m3_w / 2, vss_ty + m3_w / 2)
        _edge_route(layout, cell, "Metal2", clamp_vss_x, vss_ty, vss_rail_y, m3_w)
        rect(layout, cell, "Metal2", min(clamp_vss_x, nmos_right) - vss_rail_w / 2, vss_rail_y - vss_rail_w / 2,
